...
The [`authz.is`](/authorizer/is) API is the underlying mechanism for an application to call the 
Authorizer to make a decision. The [language SDKs](/sdks) provide higher-level abstractions for interacting 
with the authorizer.
