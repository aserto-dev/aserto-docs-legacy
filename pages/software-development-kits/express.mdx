...
[**display state map**](/authorizer-guide/display-state-map) pattern to front-ends (e.g. the 
[React SDK](/software-development-kits/react) and [Javascript SPA SDK](/software-develpment-kits/javascript)).
...
The Aserto [authorizer](/overview/authorizer) exposes HTTPS-only endpoints. In order for a node.js policy to properly communicate with the authorizer, TLS certificates must be verified.
