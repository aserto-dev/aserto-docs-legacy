...
> **Important note**: enter the Client ID and Secret for a Machine-to-Machine (M2M) Application, which has been authorized to talk to the Auth0 Management API. In the [Auth0 dashboard](https://manage.auth0.com/dashboard), select an existing **M2M application** that has access to the **Auth0 Management API**, or create a new one.  It will look something like this:
