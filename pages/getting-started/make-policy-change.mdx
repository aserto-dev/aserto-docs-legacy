...
Here's an example of a policy module:
...
Let's look at the `peoplefinder.POST.api.users._id` module. In this case,
...
In the image below, you can see that if we log in as April Stewart (Euan Garden's manager), she can now update
Euan's department and title.  
