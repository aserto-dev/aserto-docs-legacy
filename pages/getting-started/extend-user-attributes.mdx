...
Download the `users.json` file:

```bash
wget https://raw.githubusercontent.com/aserto-dev/aserto-idp-seed/main/pkg/data/users.json
```

Use the Aserto CLI to upload extended attributes for every user we created in Auth0:

```bash
aserto dir load-users --provider json --input input.json
```

Verify that the users now have extended attributes:

```bash
aserto dir list-users
```
