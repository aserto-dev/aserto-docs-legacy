...
* IDENTITY_TYPE_SUB: the user context is passed in as a subject
* IDENTITY_TYPE_JWT: the user context is passed in as a JWT
...
subject or a JWT, respectively. In the case of a JWT, the Authorizer will extract the `sub` (subject) claim, whereas 
...
Note that the `identityContext` is availabe as `input.identityContext`. If the policy wants to 
...
token = {"payload": payload} {
  [header, payload, signature] := io.jwt.decode(input.identityContext.identity)
}
